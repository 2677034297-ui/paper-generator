...
def health():
    """健康检查接口"""
    return jsonify({"status": "ok", "model": "deepseek-v4-pro"})
...
if __name__ == "__main__":
    ...
    logger.info(f"启动论文生成器服务，端口：{port}")
    app.run(
        host="0.0.0.0",
        port=port,
...
# IP 调用记录
ip_usage = defaultdict(lambda: {"date": None, "count": 0})
...
def generate():
    ...
    # 检查今日次数
    if ip_usage[user_ip]["date"] == today:
        if ip_usage[user_ip]["count"] >= 10:  # 每天10次限制
            return jsonify({"error": "今日免费次数已用完，明天再来吧"}), 429
    else:
        ip_usage[user_ip]["date"] = today
        ip_usage[user_ip]["count"] = 0
